document.addEventListener('DOMContentLoaded', () => {
    ...
    // функція для склонения числительной
    // n - число
    // t - масив із 3 варіантів склонения
    ...
    const showResult = (arr) => {
        ...
        const month = ['місяць', 'місяці', 'місяців'];
        dayThis.textContent = dayT + ' ' + declOfNum(dayT, month);
        moneyThis.textContent = moneyT + ' грн.';
    ...
    const calcResult = () => {
        ...
        if (startVal === endVal) {
            ...
            showResult(data[startVal][endVal]);
        }
    };
    ...
    document.querySelectorAll('.change_range').forEach((element) => {
